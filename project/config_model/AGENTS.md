...
## `audio.toml` — mapowanie dźwięków

Ręcznie edytowany TOML obok `routines.toml` (żadnego JSON-a do ręcznej edycji, żadnego
CSV do struktury klucz-wartość). `[music]` mapuje **nazwę mapy** albo kontekst
`main_menu`/`maze`/`death` na plik z `assets/audio/music/`, `[sfx]` mapuje **nazwę
eventu** (nie pliku!) na plik z `assets/audio/sfx/`. Instrukcja jest w komentarzu na
górze pliku, opis systemu w [`../AGENTS.md`](../AGENTS.md) (sekcja „Audio"), licencje
w `assets/audio/SOURCES.md`. Walidowany przez `just validate-world`
(`check_audio_manifest`): brakujący plik, nieznana mapa, martwy event i `play_sfx`
bez wpisu to twarde błędy.
...
nazwy miejsc na warstwie `places`, nazwy rutyn w `routines.toml`, klucze map i eventów
w `audio.toml`, katalogi sprite'ów.
