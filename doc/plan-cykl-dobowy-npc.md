...
Wersja 5: NPC wchodzą do budynków (innych map Tiled) i kontynuują tam rutyny - model timer-tranzytu + reconciler obecności. Sekcja na końcu dokumentu.
...
## Wersja 5: NPC wchodzą do budynków (rutyny cross-map)

Dotąd rutyna działała wyłącznie w obrębie jednej mapy: `resolve_at` (`npc_schedule.py:236`) czyta `scene.places` i `scene.waypoints`, które trzymają miejsca **tylko aktualnie wczytanej** mapy, a `npc.current_map` (`characters.py:113`) ustawia się raz przy spawnie i nigdy się nie zmienia. Cel wersji 5: barman idzie z Village do tawerny (mapa `VillageHouse`, LOST_CORK_TAVERN) do pracy (`VillageHouse:bar`), a mieszkańcy z rutyną `townsfolk` schodzą tam na lunch (`VillageHouse:tables`) - wszystko sterowane danymi, bez logiki per-NPC w kodzie.

Uproszczenie, świadome: gdy gracz jest na innej mapie niż NPC, silnik **nie przelicza chodzenia** tego NPC (żadnego A* dla niewczytanych map). Pamięta tylko, na której mapie NPC ma logicznie być i kiedy przechodzi między mapami. To ten sam kompromis, co znikanie NPC na noc (`sleep` opuszcza `scene.group`).

Model wybrany: **timer tranzytu** dla przejść między mapami plus **reconciler obecności** co klatkę. Zakres pełny - z poprawnym zapisem/odczytem NPC-ów, którzy w chwili zapisu są logicznie poza mapą gracza.

### Fakty z kodu, na których stoi projekt

- `loaded_NPCs` (`scene.py:943`) to jeden słownik **wszystkich** NPC sceny, tworzony leniwie z `spawn_points` każdej mapy przy pierwszym wczytaniu i trzymany na stałe.
- `self.NPCs` - lista aktywna (rysowana, aktualizowana) - budowana jest **tylko przy wczytaniu mapy** w `populate_sprite_groups` (`scene.py:532-535`), filtrem `npc.current_map == self.current_map and not npc.is_dead`.
- `update_schedule()` leci co klatkę tylko dla NPC w grupie (przez `NPC.movement`, `characters.py:724`) oraz dla śpiących (`scene.py:1506`). **NPC poza mapą gracza nie dostaje żadnego ticku** - to trzeba dodać.
- Wszyscy czterej NPC z rutyną (BARMAN, HAMMER, Johny, Bart) mają spawn na `Village` (`Village.tmx`) - mapie startowej, więc są instancjonowani od chwili startu gry. Jedyna mapa docelowa cross-map w tym scenariuszu to `VillageHouse`.
- Drzwi: Village ma exit `to_map=VillageHouse` (obj przy `x=672 y=720`); wejście do tawerny to entry_point `Door`, powrót ląduje na Village w `VillageHouseDoor`. VillageHouse ma exit `to_map=Village` oraz `places` `bar` (56,40) i `tables` (28,72).
- Istnieje martwy zaczątek `NPC.check_scene_exit` (`characters.py:1864`) z `# TODO NPC goes to another map`, który każe NPC "umrzeć" (`die(drop_items=False)`) na kolizji z exitem. Nowy mechanizm nie może z nim kolidować.

### Pułapki i problemy (poza tymi wskazanymi w zadaniu)

| # | Pułapka | Dlaczego groźna / jak rozwiązana |
|---|---|---|
| P1 | Leniwa instancja NPC per-mapa | NPC pojawiał się dopiero po pierwszym wczytaniu jego mapy spawnu, więc tick off-map nie obejmował nieutworzonych obiektów. Rozwiązane **ogólnym rejestrem**: przy starcie gry czytamy `characters.csv` i instancjonujemy każdy NPC z niepustą kolumną `routine`, niezależnie od tego, na której mapie leży jego `spawn_point` (patrz "Ogólny rejestr NPC z rutyną"). |
| P2 | `self.NPCs` odświeżane tylko przy zmianie mapy | Jeśli NPC zmieni mapę, gdy gracz stoi w miejscu, lista aktywna się nie przebuduje. Stąd reconciler co klatkę. |
| P3 | `resolve_at` waliduje po `scene.places` bieżącej mapy | Celu na niewczytanej mapie nie da się zwalidować ani zamienić na piksele. Rezolucja rozdziela się na logiczną (mapa+nazwa, zawsze) i fizyczną (piksele+A*, tylko na mapie gracza). |
| P4 | `npc.current_map` niemutowalne | Musi stać się sterowane harmonogramem (`runtime.logical_map`), inaczej filtr obecności nigdy nie ruszy NPC. |
| P5 | Zapis/odczyt buduje stany NPC tylko z `scene.NPCs` (mapa gracza) | NPC logicznie poza mapą gracza wypadłby z zapisu albo zapisał się ze stałego (nieaktualnego) snapshotu cache. Realna dziura w danych - patrz "Zapis/odczyt". |
| P6 | Który entry_point przy materializacji "w drzwiach" | Rozwiązane bez rejestru cross-map: użyć pozycji **exitu bieżącej (wczytanej) mapy prowadzącego z powrotem do mapy źródłowej** - to fizycznie te drzwi. Dosunąć `nearest_walkable` (`maze_utils.py`). |
| P7 | `check_scene_exit` NPC (`die()` na exicie) | Kolidowałoby z reconcilerem - NPC idący do drzwi "umierałby". Wyłączyć dla NPC z `routine_key`. |
| P8 | Dialog/handel pochłaniający a tranzyt | Jeśli slot wyjazdowy odpali w trakcie rozmowy/handlu, arm tranzytu trzeba zakolejkować do zamknięcia panelu - tak jak już jest z "Talk pochłaniającym" dla zmiany slotu. |
| P9 | Zegar przez północ i skok `apply_days(n)` | Czas tranzytu musi być w **absolutnych minutach gry** (`day*1440 + hour*60 + minute`), nie w minutach doby - inaczej przejście przez północ albo skok o dobę psuje timer. Tranzyt z przeszłą godziną przyjazdu domyka się natychmiast - N-bezpieczne, spójne z `apply_days`. |
| P10 | Kolizja z mechaniką `sleep` | `sleep` zostawia NPC w `scene.NPCs` (usuwa tylko z grupy). Dematerializacja cross-map usuwa też z `scene.NPCs`. Kolejność ticków musi je złożyć bez wyścigu (logika -> tranzyt -> reconciler obecności -> `update_sleepers`). |
| P11 | Nakładanie się NPC na jednym `place` | Kilku mieszkańców zbiega się w punkt `tables`. `wander` (promień) ich rozsuwa; przy `stand`/`idle` mogą stać w jednym miejscu - do odnotowania, nie blokada. |
| P12 | Stała `pos` między mapami | `pos` NPC poza mapą gracza jest bez znaczenia i przy materializacji zawsze nadpisywana. Nie wolno jej użyć jako cel A* dla obcej mapy. |
| P13 | Marsz do drzwi a moment pojawienia się | Kluczowe: **czas przyjazdu ustala się w chwili granicy slotu, niezależnie od tego, czy NPC dotarł do drzwi.** Marsz do drzwi na mapie źródłowej jest wyłącznie kosmetyczny i nie bramkuje przyjazdu. To rozwiązuje problem "gracz wyszedł, NPC jeszcze nie". |

### Model docelowy: timer tranzytu + reconciler obecności

Stan dochodzi do `NpcRuntime` (`npc_runtime.py`) - trzy pola serializowane plus jedno pole instancji:

| Pole | Rola |
|---|---|
| `logical_map` | mapa, na której NPC **logicznie** przebywa (domyślnie mapa spawnu) |
| `transit_to_map` | cel tranzytu; `""` = nie w tranzycie |
| `transit_arrive_min` | absolutna minuta gry, o której NPC pojawia się na `transit_to_map` |
| `origin_map` (nie do zapisu) | `current_map` z chwili spawnu; domyślna mapa dla destynacji bez prefiksu mapy |

#### Ogólny rejestr NPC z rutyną

Żeby tick off-map obejmował każdego NPC z rutyną - także takiego, którego `spawn_point` leży na mapie jeszcze niewczytanej - instancjonujemy rejestr przy starcie gry, odczepiony od `spawn_points`. Po wczytaniu mapy startowej (hub) iterujemy `game.conf.characters`; dla każdej postaci z niepustą kolumną `routine`, której nie ma jeszcze w `loaded_NPCs`, tworzymy obiekt NPC (nazwa = klucz modelu = `model_name` dla nazwanych postaci dialogowych). Dedup jest darmowy: `load_NPCs` już pomija `spawn_point`, którego nazwa jest w `loaded_NPCs` (`scene.py:917`), więc rejestr i spawn_point nie zdublują tej samej postaci.

`origin_map` (baza domowa NPC, punkt zaczepienia dla zapisu i domyślna mapa destynacji bez prefiksu) ustala się tak: jeśli NPC dostał `current_map` z realnego spawn_pointu na wczytanej mapie - to ta mapa; w przeciwnym razie prefiks mapy z kolumny `home`, a jak i tego brak - mapa startowa (hub). Dla obecnego scenariusza cała czwórka ma spawn na Village, więc `origin_map = Village` wychodzi naturalnie; rejestr jest tu siatką bezpieczeństwa na postać spoza huba.

Nowy tick świata poza mapą gracza - `Scene.update_routine_npcs()`, wołany raz na klatkę obok `update_sleepers` (`scene.py:1587`), iteruje **wszystkie** NPC z `loaded_NPCs`, którzy mają `routine_key`, niezależnie od mapy. Dla każdego:

1. **Tick logiczny.** Policz bieżący slot (`current_slot`, ten sam jitter per NPC). Na granicy slotu wyznacz `dest_map` przez logiczną rezolucję `at`.
2. **Arm tranzytu.** Jeśli `dest_map != logical_map`, nie w tranzycie i nie w dialogu: `transit_to_map = dest_map`, `transit_arrive_min = now_abs + transit_minutes`. Jeśli NPC jest akurat zmaterializowany na mapie gracza - ustaw mu intencję "idź do drzwi" (cel = pozycja exitu prowadzącego do `dest_map`), czysto kosmetycznie.
3. **Domknięcie tranzytu.** Jeśli w tranzycie i `now_abs >= transit_arrive_min`: `logical_map = transit_to_map`, wyczyść pola tranzytu.
4. Same-map off-map (`dest_map == logical_map`): nic więcej - dokładna pozycja nieistotna, dopóki gracz nie wejdzie na tę mapę.

Reconciler obecności, w tej samej pętli, po logice, dla mapy gracza:

- NPC z `logical_map == player_map`, **nie** w tranzycie, **nie** w `self.NPCs` -> **materializuj**:
  - tranzyt domknął się przy graczu obecnym na tej mapie -> `pos` = pozycja exitu prowadzącego do mapy źródłowej (`nearest_walkable`), cel = miejsce ze slotu, `find_path()` (NPC "wchodzi w drzwi i idzie do celu");
  - NPC był już logicznie tutaj w chwili wczytania mapy (gracz wszedł po fakcie) -> `pos` = piksele miejsca ze slotu (`nearest_walkable`), bez marszu (akceptowany "teleport do celu").
- NPC w `self.NPCs`, ale `logical_map != player_map` lub w tranzycie -> **dematerializuj** (usuń z `self.NPCs` i z grupy, zachowaj w `loaded_NPCs`, runtime nietknięty).

Materializacja i dematerializacja to ta sama higiena co w `update_sleepers` (add/remove z grupy warstwami), więc wzorzec już istnieje.

Dlaczego to rozwiązuje problem "gracz wyszedł, a NPC jeszcze nie": `transit_arrive_min` ustala się **w chwili granicy slotu wyjazdowego**, nie w chwili dotarcia do drzwi. Marsz do drzwi na mapie źródłowej jest kosmetyczny i zostaje ucięty przez dematerializację (gracz zmienił mapę / NPC dotarł do drzwi / minął timer - co pierwsze). Odpowiedź na "kiedy NPC ma się pojawić na Village" brzmi zawsze: o `transit_arrive_min` policzonej z góry - więc gracz może wejść na Village przed, w trakcie albo po tym czasie, a reconciler zawsze policzy spójny stan.

### Rozszerzenie gramatyki `at` i kolumn CSV

Gramatyka wartości destynacji: `[<mapa>:]<nazwa>`. Brak prefiksu mapy = **mapa spawnu NPC** (`origin_map`), więc wszystkie dotychczasowe jednomapowe rutyny działają bez zmian.

| Zapis | Rozwiązanie |
|---|---|
| `type:work` | kolumna `work` postaci; jej wartość to `[mapa:]miejsce` (np. `VillageHouse:bar`) |
| `location:VillageHouse:bar` | wprost mapa+miejsce z warstwy `places` mapy docelowej |
| `route:Village:patrol_north` | mapa+trasa z warstwy `waypoints` mapy docelowej |

Parsowanie: `at.partition(":")` daje prefiks (`type`/`location`/`route`) i resztę; reszta przez `partition(":")` daje `mapa` i `nazwa` (dla `type` mapa bierze się z wartości komórki CSV, nie z `at`). `resolve_at` zwraca `Destination(kind, name, map)`. Walidacja `known_places`/`known_routes` obowiązuje **tylko** gdy `map == current_map`; dla obcej mapy rezolucja jest logiczna (bez walidacji pikseli) - zasada "półzmapowana wioska ma być grywalna" zostaje utrzymana.

### Zapis / odczyt (najtrudniejsza część)

Problem P5: `_build_npc_states` (`save_load/manager.py:271`) czyta tylko `scene.NPCs` (mapa gracza), a NPC z rutyną bywa logicznie gdzie indziej. Rozwiązanie stoi na fakcie, że NPC z rutyną **zawsze** żyją w `loaded_NPCs` (spawn na Village):

- Zapisuj każdy NPC z rutyną z **żywego obiektu** z `loaded_NPCs`, przypięty do jego `origin_map` (Village), a nie ze stałego snapshotu cache. Konkretnie w `_build_map_states` dla mapy origin preferuj żywy obiekt nad cache dla NPC z `routine_key`.
- `NpcRuntime` (już serializowany przez `NPCState.runtime` deepcopy) niesie `logical_map` i pola tranzytu; `from_dict` je odczytuje. Stare zapisy dostają wartości domyślne (`logical_map=""` traktowane jak `origin_map`).
- Po wczytaniu reconciler odtwarza obecność i pozycję z `logical_map` + tranzytu; zapisany `pos` używany tylko gdy NPC jest osiadły na `origin_map`.

### Zmiany w plikach

Modyfikowane:

- `project/npc_runtime.py` - pola `logical_map`, `transit_to_map`, `transit_arrive_min` + aktualizacja `from_dict`.
- `project/npc_schedule.py` - `Destination` zyskuje pole `map`; `resolve_at` parsuje `[map:]name`, przyjmuje `origin_map`/`current_map`, waliduje piksele tylko dla mapy bieżącej.
- `project/characters.py` - `origin_map` i init `logical_map` w `__init__`; rozdział `update_schedule` / `_begin_slot` na logiczny (mapa) i fizyczny (piksele+A*, tylko gdy `dest.map == scene.current_map`); wyłączenie `check_scene_exit` dla NPC z rutyną.
- `project/scene.py` - `update_routine_npcs()` (tick off-map + reconciler), helper `abs_minutes()`, helper "pozycja drzwi z mapy X" (pozycja exitu, którego `to_map == X`, `nearest_walkable`), przełączenie filtra w `populate_sprite_groups` z `current_map` na `logical_map`.
- `project/save_load/manager.py` - zapis NPC z rutyną z żywego obiektu, przypięty do `origin_map`.
- `project/npc_state.py` - arm tranzytu objęty tym samym pochłanianiem co zmiana slotu w trakcie rozmowy.
- `project/config_model/characters.csv` - `BARMAN_ABSINTHRAYNER`: `work = VillageHouse:bar`; `HAMMER_HOAXHEART` (i wg uznania Johny/Bart): `social = VillageHouse:tables`. Po edycji: `just import-entities`.
- `project/config_model/routines.toml` - `[defaults] transit_minutes` (domyślnie ~30 minut gry = ~2 s realne przy `GAME_TIME_SPEED=0.25`).

Dane Tiled: `VillageHouse.tmx` ma już `places` `bar`/`tables`; upewnić się, że exit `to_map=Village` stoi przy progu. `Village.tmx` - `places` dla `home`/`work` (część już jest); brakujące miejsce = "zostań gdzie jesteś", więc można uzupełniać po jednym.

### Kolejność budowy

Status implementacji: **wszystkie 8 kroków zrobione** (68 testów `test_npc_schedule.py` + `test_save_load_routine_npc.py`, brak regresji w 30 plikach testów, czysty boot headless także z aktywnymi danymi cross-map). Dane w CSV wpięte (barman `work=VillageHouse:bar`, mieszkańcy `social=VillageHouse:tables`), `config.json` przeimportowany (czysty diff 5 linii - bez reformatu, bo plik jest już 4-spacjowy; przy okazji naprawiona rozjazd: barman w `config.json` miał `routine=townsfolk` zamiast `barman`). Zostaje **weryfikacja wizualna u autora**: barman od 9:00 idzie do tawerny do pracy, mieszkańcy schodzą na lunch (jitter rozsuwa ich 13:00/14:00).

1. ~~`NpcRuntime` + `origin_map` + `resolve_at` z wymiarem mapy~~ - **zrobione**. Pola `logical_map`/`transit_*`, gramatyka `[map:]name`, `split_map`/`slot_target_map`, `Destination.map` (default `""` = wstecznie zgodne).
2. ~~`Scene.update_routine_npcs()` - tick logiczny + arm/domknięcie tranzytu, `abs_minutes()`~~ - **zrobione**. Czysta `step_logical_map` + `transit_minutes` w `[defaults]`.
3. ~~Ogólny rejestr NPC z rutyną~~ - **zrobione**. `load_routine_roster` (raz po pierwszym `load_map`), `routine_roster_keys`/`roster_origin_map`, sprite'y w grupach-śmietnikach, dedup ze `spawn_points`.
4. ~~Reconciler obecności + helper pozycji drzwi + filtr~~ - **zrobione**. `reconcile_routine_presence` (co klatkę), `_settle_routine_npcs` (w `populate_sprite_groups`), `_materialize_routine_npc`/`_dematerialize_routine_npc`, `exit_to` (drzwi = exit prowadzący do mapy źródłowej), `_slot_place`/`_walkable_pixel`. Filtr został na `current_map`, synchronizowanym z `logical_map` (mniej zmian w `populate`).
5. ~~Rozdział `_begin_slot` + wyłączenie `check_scene_exit` dla rutyny~~ - **zrobione**. Guard w `_begin_slot` (cel na obcej mapie = nic fizycznego) i early-return w `NPC.check_scene_exit` (inaczej NPC ginie na drzwiach, na których materializuje się przychodzący).
6. ~~Dialog pochłaniający obejmuje arm tranzytu~~ - **zrobione za darmo**. Modal (dialog/handel) robi early-return w `Scene.update` przed `update_routine_npcs` i przed zegarem, więc czas stoi i żaden slot ani tranzyt się nie rusza w trakcie rozmowy.
7. ~~Zapis/odczyt off-map~~ - **zrobione**. `_pin_routine_npcs_to_origin` (zapis: każdy NPC z rutyną raz, pod `origin_map`, z żywego obiektu, dedup) i `_apply_routine_npc_states` (odczyt: stan wraca na żywy `loaded_NPCs` nawet gdy NPC jest off-map). Pozycja i tak leczy się z zegara; kluczowe jest, że sakiewka/stock/dialog handlarza off-map nie resetują się do configu. Testy: `test_save_load_routine_npc.py`.
8. ~~Dane: CSV + `import-entities`~~ - **zrobione**. `BARMAN work=VillageHouse:bar`, `JOHNY`/`BART`/`HAMMER social=VillageHouse:tables` (miejsca `bar`/`tables` już na `VillageHouse`, domy/stragany już na `Village`). `import-entities` dał czysty diff 5 linii. Pozostaje weryfikacja wizualna u autora + ewentualny tuning: slot `social` townsfolk trwa 1 h gry, a `transit_minutes=30` zjada połowę na dojście - może warto wydłużyć okno lunchu albo skrócić tranzyt.

### Weryfikacja

| Co | Jak |
|---|---|
| Tick logiczny | Test: `current_slot` -> `dest_map`; granica slotu cross-map ustawia `transit_arrive_min = now_abs + transit_minutes`; przez północ i przy `apply_days(n)` domyka się poprawnie. Bez pygame. |
| Gramatyka `at` | `type:`/`location:`/`route:` z prefiksem mapy i bez; brak mapy = `origin_map`; obca mapa nie waliduje pikseli i nie rzuca. |
| Scenariusz na żywo (`agent_ctrl`) | Ustaw godzinę ~8:00: barman rusza do tawerny, znika w drzwiach, po `transit_minutes` jest na `bar`. Ustaw ~13:00: kowal schodzi na lunch do `tables`. |
| "Gracz wyszedł, NPC jeszcze nie" | Wejdź do tawerny pod koniec lunchu, zobacz jak NPC rusza do drzwi, wyjdź przed nim na Village, poczekaj do `transit_arrive_min` - NPC pojawia się na Village (przy `VillageHouseDoor` idąc do celu albo już osiadły). |
| Wejście po fakcie | Wejdź do tawerny grubo po starcie lunchu - NPC są od razu przy `tables` (teleport, akceptowany). |
| Zapis/odczyt | Zapis gdy NPC off-map lub w tranzycie; po wczytaniu obecność i pozycje się zgadzają. |
| Wizualnie | U usera - zrzuty headless nie są wierne dla kompozycji klatki i paneli modalnych. |

### Bug z pierwszego playtestu: materializacja na kaflu drzwi

Barman poprawnie stał przy barze, ale **przy powrocie NPC lądowali dokładnie na kaflu drzwi i utykali** - bo `exit_to()` zwracał pozycję kolizji exitu, a ten kafel jest ścianą (wchodzi się *w* niego, żeby wyjść). NPC postawiony na ścianie zamraża się: A* nie potrafi zejść z zablokowanego startu (`nearest_walkable` dosuwa tylko *cel*, nie start).

Poprawny punkt to **entry_point po tej stronie drzwi** - ten sam, na którym ląduje gracz przechodząc (tu `VillageHouseDoor`), leżący na przejezdnym kaflu tuż obok progu. Nowy `_arrival_pos(source_map)` bierze entry_point **najbliższy** exitowi prowadzącemu z powrotem do mapy źródłowej (drzwi i ich entry_point to ten sam próg), z fallbackiem na `_walkable_pixel`. Heurystyka "najbliższy" jest odporna, bo drzwi są przestrzennie rozdzielone: na `Village.tmx` exit→VillageHouse (672,720) trafia w `VillageHouseDoor` (680,752, ~33 px), exit→JacobsChamber w `JacobsChamberDoor`, exit→Maze w `NextToMaze` - każdy w swój właściwy punkt. Bez rejestru cross-map i bez czytania niewczytanej mapy źródłowej.

### Bugi z drugiego playtestu: klony i znikanie w miejscu

Trzy sprawy naraz.

**Klony przy straganach.** Spawny mają nazwy `Johny`/`Bart`, ale przez `gid` dziedziczą `model_name=JOHNY`/`BART` z kafla, więc `loaded_NPCs` jest kluczowane `"Johny"`, a `config_key="JOHNY"`. Rejestr deduplikował po nazwie obiektu (`"JOHNY" not in {"Johny"}`) i tworzył klon: klon (origin=Village) materializował się od razu przy straganie, oryginał do niego szedł. Barman/Hammer nie klonowały się (nazwa == klucz). Fix: dedup po `config_key`. Zweryfikowane na danych: wszystkie 4 postacie z rutyną spawnują na Village → rejestr tworzy 0.

**Za krótki lunch.** Slot `social` townsfolk trwał 13:00-14:00, a marsz + tranzyt zjadał większość - gracz nie zdążył wejść. Powrót do pracy przesunięty na 16:00.

**Znikanie w miejscu przy wyjściu na lunch.** Sedno: `_routine_physical_map` zwracał sentinel "w tranzycie" w chwili uzbrojenia, a reconciler (ta sama klatka) dematerializował NPC **natychmiast**, zanim ruszył. Poprawny model: **czas przyjazdu na drugą stronę liczy się od momentu przejścia przez drzwi, nie od granicy slotu.** Odchodzący NPC zostaje widoczny na mapie źródłowej (`logical_map` trzyma się źródła), dostaje cel = drzwi (`exit_to`) i idzie; flaga `_transit_gone` zapala się dopiero, gdy dojdzie do progu (`is_travelling` gaśnie) - wtedy znika, a `transit_arrive_min` przestawia się na `now + transit_minutes` (krótkie, ~1 s), więc pojawia się przy stołach chwilę później. `_DEPARTURE_FALLBACK_MIN` (240) to tylko podłoga na wypadek, gdyby ścieżka do drzwi się nie znalazła. Wyjście poza mapą gracza: `_transit_gone` od razu (nie ma czego pokazywać). `transit_minutes` zmienia znaczenie: to czas "od drzwi do celu na drugiej mapie", nie długość całej podróży.

Zweryfikowane bezpośrednim krokowaniem sceny Village przez lunch (patrz niżej): Jaś rusza ~12:42, idzie do drzwi ~2,8 s, znika w progu, jest w tawernie ~13:39; Bartuś analogicznie; zero "Path not found".

### Weryfikacja headless: krokować scenę, nie bootować grę

**Boot `main.py run` ląduje w `MainMenuScreen`, nie w `Scene`** - więc "boot smoke" (uruchom i sprawdź brak tracebacku) **nie wykonuje** logiki rutyn ani rozgrywki. Żeby zweryfikować zachowanie NPC headless bez menu: skonstruuj scenę wprost i krokuj ją -

```python
import scene; scene.INITIAL_HOUR = 12          # patch kopii w namespace scene
from game import Game; from scene import Scene
g = Game("run", []); sc = Scene(g, "Village", ""); sc.enter_state()
for _ in range(400):
    g.time_elapsed += 0.1
    sc.update(0.1, [])                          # zegar rusza sam (GAME_TIME_SPEED)
    # ...inspekcja sc.loaded_NPCs / sc.NPCs / runtime...
```

`entry_point=""` daje nieszkodliwe "no entry point found" dla gracza - bez znaczenia dla NPC. To jedyny wierny sposób sprawdzenia cyklu dobowego bez prawdziwego ekranu.

### Znane ograniczenia (świadome)

- **Teleport przy wejściu po fakcie** - świadomy kompromis; brak animacji dojścia od drzwi, gdy gracz wchodzi długo po starcie slotu.
- **`transit_minutes` globalne** - jedna wartość dla wszystkich drzwi. Per-link i zależność od realnej odległości między mapami to refinement.
- **Nakładanie się NPC na jednym `place`** (`tables`) - rozsuwa `wander`, ale przy `stand`/`idle` mogą stać w jednym punkcie.
