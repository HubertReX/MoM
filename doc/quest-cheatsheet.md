...
Jeden plik = jeden główny ==quest==. Nagłówek sekcji jest **kluczem** questa, dosłownie, i musi być globalnie unikalny. Alias to **własny klucz parasola** - sekcji, której podlegają wszystkie
pozostałe w pliku.
...
ignorowane z ostrzeżeniem - dzięki temu plik **EN** można bezpiecznie wygenerować LLM-em: najgorsze, co zrobi, to źle napisana proza, nigdy zepsuty quest.

## Requires - zależności między questami

Link do questa, który musi być ukończony aby **odblokować** ten krok. Klucz to tekst po ostatnim `#`, więc każdy poniższy zapis działa to samo:
...
- `all_subquests` bez kroków - nic by jej nigdy nie zamknęło.
...
Nie ma - `sentiment` działa to tylko w dialogu, bo ==quest== nie ma kontekstu bieżącej postaci.
...
- **Po lewe**j: coś, co zwraca **liczbę**. W praktyce `item_count("ITEM")` - to jedyny na razie predykat, który zwraca liczbę, a arytmetyki (`+`, `*`) whitelista nie przepuszcza.
- **Po prawej**: liczba całkowita oznaczająca oczekiwaną wartość do spełnienia. Naleyż podać oba albo żadne - `Postęp:` bez licznika to błąd importu.
- Wartość bieżąca (**po lewej**) jest przycinana do zakresu, a ukończony quest zawsze pokazuje wtedy pełny pasek.

> [!warning] Pasek postępu to nie to samo co warunek ukończenia
> Quest z paskiem `3 / 3` **nadal się nie zamknie**, dopóki nie napiszesz `Test:`.
> Pasek postępu mówi ile brakuje; `Test:` decyduje kiedy jest gotowe. Zwykle chcesz obu:
>
> ```markdown
> **Completion**: test
...
Parasole (`all_subquests`) dostają pasek **za darmo**, liczony z kroków - nie dodawaj dla nich
`Postęp:`.
...
| `max_items=nn`          | sloty w pasku (limit `MAX_HOTBAR_ITEMS_LIMIT=8`) | `max_items7`                           |
...
```bash
just import-quests # importuje wszystkie łańcuchy do config.json; Qxx albo pełny klucz = importuje tylko ten jeden
