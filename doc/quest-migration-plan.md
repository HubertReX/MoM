...
- **Treść (Q-10)** - proza i nagrody to szkic, do przepisania Twoim głosem.
- **`Q01_S05` używa węzła `001`** - klucz z planu nie istniał w MoM (szczegóły w Q-10).
- **Znaczenie filtrów w panelu** (Q-08) - przyjąłem "Aktywne = odblokowane i nieukończone".
- **`Q01_S00_BREAK_THE_CURSE` nie da się dziś zamknąć** - znalazł to graf z Q-11. Jest
  `manual`, a `manual` domyka wyłącznie `mark_done()` z kodu gry; jedyne wywołanie
  `mark_done` w repo siedzi w silniku (`engine.py`) i obsługuje `test`/`all_subquests`.
  Wątek klątwy nie jest przez to zablokowany (bramka po `parent` patrzy na *odblokowanie*,
  nie na *domknięcie*), ale główny quest gry zostanie otwarty na zawsze. Świadoma decyzja
  czy TODO - Twój wybór.
...
| Q-12 | Ergonomia autorska w vaultcie | #L | Q-04, Q-08 |
...
**Zrobione 2026-07-16.** Pliki: `project/quest/rewards.py`, rozszerzony `ResultSink`
+ `GameResultSink`, refaktor `max_items`, `[quest]` w locale, testy
`tests/test_quest_rewards.py` (8).

#### Trzy decyzje (zamknięte 2026-07-16)

| Pytanie | Wybór | Konsekwencja |
| --- | --- | --- |
| `sentiment` - w co celuje? | **Nagroda wskazuje NPC** | Składnia `sentiment=10 @BARMAN_ABSINTHRAYNER`; `QuestReward.target`; nowa metoda `shift_sentiment_of(npc_key, amount)` |
| `max_items` - kiedy refaktor? | **Teraz, w Q-05** | `MAX_HOTBAR_ITEMS` → pole `NPC.max_items`; HUD przelicza się w runtime |
| `raise_max_health` - co ze zdrowiem? | **Podnosi też bieżące o tyle samo** | 80/80 → +20 → 100/100; poobijany 50/80 → 70/100 |

**Podwójna nagroda (Pułapka 7) nie wymagała decyzji silnika.** Każdy quest aplikuje swoje
nagrody dokładnie raz przy ukończeniu; czy parasol płaci "ponad" krokami, wynika wyłącznie
z treści. Konwencja przyjęta w Q-10: kroki nie płacą, płacą parasole.

Rozstrzygnięcia techniczne:

- **`sentiment` szuka NPC w trzech miejscach**, jak `find_visited_node`: bieżąca mapa → cache
  map z tej sesji → `pending_map_states`. NPC nigdy niespotkany → **głośny log**, nie ciche
  pominięcie nagrody.
- **Sufit `MAX_HOTBAR_ITEMS_LIMIT = 8` nie jest arbitralny.** Każdy slot potrzebuje ikony
  `key_<n>` i bindu `INPUTS["item_<n>"]`. `generate_icons()` renderuje cyfry 0-8, a `ACTIONS`
  miało `item_1`..`item_6` - dodałem `item_7`/`item_8`. Powyżej 8 hotbar narysowałby slot,
  którego nie da się wybrać. `raise_max_items` klampuje, load też (ręcznie zedytowany save
  z 99 slotami nie rozjedzie UI).
- **`hotbar_topleft(slots)` liczy się co klatkę**, nie raz w konstruktorze. Pasek policzony
  przy starcie zostałby wyśrodkowany na starej szerokości i uciekłby w lewo.
- **`max_items` i `damage` idą do save'a** - inaczej nagroda parowałaby po wczytaniu. Pola
  z defaultami, więc stare save'y ładują się dalej.
- **`GameResultSink` musiał zaimplementować nowe metody.** `ResultSink` to Protocol, po którym
  adapter dziedziczy jawnie - niezaimplementowana metoda zwracałaby cicho `None` zamiast
  wywalić się. To ta sama pułapka co przy `NPCConditionContext` w Q-02.

**Weryfikacja na żywej grze:** 4 nagrody → 4 efekty (money 20→120, max_health 80→100,
damage 10→15, max_items 6→8); zdrowie idzie za maksimum (80/80 → 100/100); sufit trzyma na 8;
wszystkie bindy i ikony dla 8 slotów istnieją; HUD rysuje 8-slotowy pasek i przesuwa lewą
krawędź 376 → 288 (wyśrodkowany); po save/load `max_items=8`, `damage=15`, `max_health=100`.
Etykieta: `+100 💰 · +20 max HP · +5 obrażeń · +2 slot(ów)`. Smoke: "Save and Load Basic"
i "UI Flow - Full Save Load" - PASS.
...
**Zrobione 2026-07-16.** Pliki: `project/ui/panels/quest.py`, `quest_log` w `settings.py`,
wpięcie w `ui/game_ui.py`, 12 etykiet + `action.quest_log` w locale, testy
`tests/test_quest_panel.py` (9). Zbudowane na współrzędnych i palecie z sekcji 6 HTML-a.

#### Czego nie dało się przenieść z makiety 1:1

Makieta była rysowana w foncie proporcjonalnym, a MoM renderuje w **pikselowym**. To nie jest
kosmetyka - trzy rzeczy wymagały decyzji, wszystkie widoczne dopiero na screenshocie z gry:

- **`✔ ● ○ ▾` nie istnieją w tym foncie.** Zweryfikowane pomiarem: wszystkie cztery renderują
  **identyczną liczbę pikseli** (84150), czyli ten sam prostokąt tofu. Tak samo strzałki
  `↑ ← →` (71910). Rysuję je więc **kształtami** (`pygame.draw`) - ten sam legendarny zestaw,
  w każdym foncie i rozmiarze. Strzałki w stopce zastąpiłem literami: klawisze i tak są `W/S`
  i `A/D` (`ACTIONS` mapuje `K_UP`/`K_w`), więc podpowiedź jest teraz *bardziej* prawdziwa.
- **Pozycje liczone pomiarem, nie na sztywno.** Licznik ("8 zadań · 4 ukończone") przy stałym
  `x` z makiety wchodził w tytuł, a filtr "Wszystkie" wypadał za panel. Teraz filtry są
  wyrównane do prawej jako blok, a licznik przycina się do tego, co zostało (i jest w `TINY`,
  bo w `SMALL` robiło się z niego "4 uko...").
- **Tytuły są przycinane wielokropkiem.** Bez tego długi tytuł questa przechodził przez
  przegrodę i lądował na tekście prawej kolumny.

#### Kolejność rysowania

Panel na pełen ekran musiał wejść **nad** HUD: skrzynka ze statami rysowała się na tytule
dziennika. `draw_overlay(stats=False)` przy otwartym panelu; **powiadomienia zostają** - to
ulotna informacja i gracz nie powinien jej tracić za to, że ma otwarty dziennik. `QuestPanel`
jest w `_BLOCKING` (hotbar nie prześwituje) i w `_MODAL` (strzałki sterują listą zamiast chodzić
bohaterem, a `Esc` zamyka panel zamiast otwierać menu).

#### Filtry

| Filtr | Znaczenie |
| --- | --- |
| Aktywne | wątki odblokowane i **nieukończone** - to, nad czym można teraz pracować |
| Ukończone | wątki ukończone |
| Wszystkie | wszystko, łącznie z zablokowanymi |

Makieta pokazuje pod "Aktywne" także ukończony `Q00` - to ilustracja wszystkich trzech stanów
ikon naraz, nie definicja filtra. Przyjąłem znaczenie użyteczne (**do korekty, jeśli wolisz inaczej**).

**Zablokowane kroki (`○`) są widoczne** - zgodnie z ustaleniem. Jest na to test z komentarzem
tłumaczącym dlaczego: dawkowanie fabuły to zadanie autora, nie panelu.

**Weryfikacja:** screenshot z prawdziwej klatki gry (`Village`, wątek Q03 na 2/3) - dwie rundy
poprawek na podstawie tego, co realnie widać. Panel pokazuje 8 questów pod "Wszystkie", postęp
**2/3** zgadza się z makietą, zwijanie działa. Smoke: "Save and Load Basic" i "UI Flow - Full
Save Load" - PASS.
...
klasy `mom-*`, podświetlanie sąsiadów, toolbar). **Nie** reużyty jest model `DialogGraph` -
quest nie ma opcji, sentymentu ani krawędzi resume, więc przeginanie go przez tamten kształt
kosztowałoby więcej, niż daje. Graf buduje `init_quests()` z `config.json`, czyli **ten sam
builder co gra**: config, który się tu narysuje, gra uruchomi.

Doprecyzowania podjęte przy Q-11:

- **Jedna notatka na wszystkie łańcuchy** (dialogi mają jedną na postać). Sens grafu leży
  w krawędziach *między* łańcuchami - `Q03_S00 requires Q01_S01` jest niewidoczne w każdym
  pliku z osobna.
- **Ranga = najdłuższa ścieżka odblokowań**, nie BFS. Quest czeka na **wszystkie** swoje
  `requires`, więc najwcześniejsza możliwa ranga to `max(rangi zależności) + 1`. BFS stawiał
  `Q01_S05` na randze 2 - obok `Q01_S01`, na który ten quest czeka - czyli rysował kłamstwo,
  że oba otwierają się razem. Rekurencja jest bezpieczna, bo `init_quests` już udowodnił
  acykliczność.
- **Hierarchia, nie fizyka** (odwrotnie niż w grafie dialogów). Tam `sortMethod: "directed"`
  gubił rangi przez cykle `resume`; tu graf jest acykliczny z walidacji, więc rangi są
  uczciwe. Fizyka (barnesHut) została jako przełącznik "Układ: swobodny".
- **Kolor = tryb `completion`**, nie stan. Panel (Q-08) koloruje po stanie (zrobione/aktywne/
  zablokowane); graf nie ma savegame'a i nie może udawać, że wie - więc odpowiada na inne
  pytanie inną paletą.
- **`uncloseable()` to jedyna kontrola, której `validate_references()` zrobić nie może.**
  Reszta roli "wykrywacza" faktycznie przeszła do importu (Q-04). Zostało pytanie leżące
  w szczelinie między configiem a kodem: `manual` znaczy "zamknie ją `mark_done()`", a tego
  wywołania żaden walidator configu nie widzi. Sprawdzenie propaguje się przez parasole
  (`all_subquests` nad krokiem, którego nic nie zamyka, to wątek bez końca) i **zgłasza,
  nie wywala** - nie podpięty `manual` to TODO, a nie błąd. Pierwsze uruchomienie od razu
  wskazało `Q01_S00_BREAK_THE_CURSE` (patrz "Stan").

**Bug złapany przez obejrzenie, nie przez test:** w trybie hierarchicznym `fit()` nigdy się
nie wykonywał - kod czekał na `stabilizationIterationsDone`, a przy `physics: false` to
zdarzenie nie pada. Graf siadał niedopasowany. Do tego domyślny `fit()` nie przybliża powyżej
skali 1, więc mały graf wypełniał płótno w 1/3 (zmierzone: 33% x 67% -> 45% x 91% po
`maxZoomLevel: 2`).

### Q-12 · Ergonomia autorska w vaultcie `#L` ✅

**Zrobione 2026-07-17**, po pierwszym realnym pisaniu treści. Sześć zgłoszonych rzeczy,
plus trzy bugi znalezione po drodze. Commity: `960f048`, `54d1748`, `25fa466`, `45bce99`,
`fff09eb`, `e812215`, `e2d0b9a`.

**Schemat kluczy.** Nagłówek sekcji **jest** kluczem questa, dosłownie - koniec sklejania
`<alias>_<sekcja>`. Alias = własny klucz parasola, więc znika magiczna stała
`_UMBRELLA_SECTION = "S00"`, a wikilink między łańcuchami rozwiązuje się przez alias i
...
z ukończenia Q00: 0.0 / 1.2 / 2.4 s, każdy z pełnymi 5 s.

**`[/]` zamyka ostatni otwarty tag.** Bezpieczne, bo sprawdzone: `[/]` nie występowało w niczym,
co trafia do gry (0 w CSV-kach i w `messages`) - tylko w notatkach prozą, których importer nie
czyta. Parser przeszedł na grupy nazwane, bo dołożenie gałęzi przesuwało emoji z `group(4)`
na `group(5)`.

**Tytuł, opis i sukces przez RichText**, w grze i w tooltipie grafu (tam wszystko spłaszcza się
do pogrubienia - tooltip jest w motywie czytelnika i nie ma palety MoM). `markup.cut_markup()`
przycina tekst nie psując tagów; panel dobiera długość binary searchem mierzonym samym
RichTextem, bo `[bold]` zmienia advance i ręczna arytmetyka rozjechałaby się z renderem.

**Trzy bugi znalezione po drodze** - szczegóły niżej, w "Bugi UI (znalezione przy Q-12)".

## Bugi UI (znalezione przy Q-12)

Żadnego z nich nie widać w teście na stringach; wszystkie wyszły z obejrzenia renderu.

### Ikony w chipach nagród i w toastach nie rysowały się w ogóle

**Dwie niezależne przyczyny.** Po pierwsze `:golden_coin:` nie był obrazkiem: `markup._EMOJIS`
bierze wyłącznie `EMOTE_SHEET_DEFINITION`, a moneta siedzi w `ITEMS_SHEET_DEFINITION` - parser
robił z niej tekst i chip pokazywał dosłowne `:golden_coin:`. Po drugie, i groźniej,
**`content_surface` nie zawiera ikon**: `_bake()` blituje do niego sam tekst, obrazki lądują
w `image_items` i rysuje je dopiero `draw()`, żeby móc animować. Panel i HUD cache'owały
`content_surface` i blitowały go same, więc gubiły **każdą** ikonę - layout rezerwował
szerokość, nikt jej nie wypełniał, i dlatego nikt tego nie zauważył.

**Zasięg szerszy niż questy:** żaden toast w grze nigdy nie pokazał swojej ikony typu
(`:blessed:` przy sukcesie, `:exclamation:` przy ostrzeżeniu). Regresja z migracji na RichText,
dużo starsza od questów.

Naprawa: `parse(..., extra_emojis=...)` (opt-in per miejsce wywołania - nazwa w tym zbiorze
**bez** ikony renderuje się jako nic, więc kto poszerza zbiór, ten dostarcza ikony) plus
`RichText.render_static()` dla tych, co cache'ują i blitują sami.

### Ścieżka błędu parsera sama się wywalała

Zepsuta **od początku** (potwierdzone `git stash`-em na starym kodzie). Komunikat szedł przez
`rich.print`, które czytało `[/char]` w jego własnej treści jako swój markup i rzucało
`MarkupError` - czyli literówka w tagu w dowolnym dialogu wywalała się w handlerze napisanym
po to, żeby ją zgłosić.

### Proza z `**Sukces**:` nigdy nie docierała do gracza

Importowana, trzymana w `config.json` i tłumaczona na dwa języki od Q-10, a `success_text()`
z `quest/rewards.py` nie było wołane z żadnego miejsca w grze - tylko z własnego testu.
Teraz: sekcja WYNIK w panelu (tylko dla **ukończonych** - proza *jest* odpowiedzią i pokazana
z góry psuje krok, który ma na nią zapracować) oraz proza w toaście ukończenia.
