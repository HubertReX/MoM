...
Wszystkie **15 decyzji zamkniętych** (2026-07-16). Kod: **Q-01 i Q-02 zrobione** (2026-07-16),
reszta nie zaczęta. Następny krok: Q-03 (silnik) lub Q-04 (import MD).

Po drodze przy Q-02 wyszedł **bug w save/load niezwiązany z questami** - patrz "Bug save/load
(znaleziony przy Q-02)" niżej. Był naprawiony jako warunek wstępny, bo bez tego `visited()`
cross-mapowe nie mogło działać.
...
- Acykliczność `requires` zostaje w Q-03, jak w planie.
...
## Bug save/load (znaleziony przy Q-02)

**Nie dotyczy tylko questów** - znaleziony przy projektowaniu cross-mapowego `visited()`,
naprawiony 2026-07-16 jako warunek wstępny Q-02.
...
**Weryfikacja:** `tests/test_save_load_multi_map.py` (5 testów; sprawdzone, że **failują na starym
...
**Zostaje otwarte (nie ruszane):** ta sama ścieżka dla **bieżącej** mapy dokleja zapisane
przedmioty do tych zrespawnowanych z TMX-a (`_apply_map_states` → `_apply_ground_items` bez
czyszczenia). Wygląda na duplikowanie itemów po load, ale to osobny bug sprzed questów -
do potwierdzenia i osobnego taska.
