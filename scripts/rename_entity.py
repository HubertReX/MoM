...
class Source:
    ...
    def paths(self) -> list[Path]:
        untouched = untouched_paths()
        return sorted(p for p in REPO_ROOT.glob(self.glob) if p not in untouched)
...
#: **Świadome wyjątki.** Klucz jest ŚCIEŻKĄ ALBO GLOBEM względem `REPO_ROOT`, bo część
#: wyjątków to reguła, a nie jeden plik ("każdy `*_base.tmx`"). Ta lista wygrywa z globami
#: z `SOURCES`: plik, który tu trafi, przestaje być przeszukiwany i przemianowywany.
UNTOUCHED_SOURCES: dict[str, str] = {
    "project/assets/NinjaAdventure/maps/**/*_base.tmx":
        "prototyp-katalog klocków dla generatora map (`just map-palette`) - jego obiekty "
        "to metadane klocków, nie encje gry; rename encji nie ma tu czego szukać",
...
def untouched_paths() -> set[Path]:
    """Pliki wyłączone przez `UNTOUCHED_SOURCES` - klucz może być ścieżką albo globem."""
    return {path for pattern in UNTOUCHED_SOURCES for path in REPO_ROOT.glob(pattern)}
