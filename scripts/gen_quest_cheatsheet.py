...
def _arrows_table() -> str:
    """Cztery rodzaje krawędzi grafu, opisane polami z notatki questa.

    Nie wyprowadzone z kodu jak reszta - `quest_graph.py` trzyma te style w
    JS-ie notatki Dataview, a nie w strukturze, którą dałoby się tu zaimportować.
    Przechodzi przez `table()` mimo to, bo dopełnienie kolumn ma trzymać się samo:
    ręcznie wyrównana tabela rozjeżdża się przy pierwszej poprawce słowa.
    """
    return table(
